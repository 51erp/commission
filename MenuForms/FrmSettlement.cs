...
namespace Commission.MenuForms
{
    public partial class FrmSettlement : Form
    {
        ...
        public DataTable GetContractData(string closingDate, out int iBindQuantity)
        {
            ...
            string condition = string.Format(" select ContractID from Receipt where SettleState = 0 and Convert(Varchar(10),RecDate,120) <= '{0}' and ProjectID = {1} ", closingDate, Login.User.ProjectID);

            //收款日期小于截止日期且未结算
            sql = "select a.ContractID, a.ContractNum, CustomerName, b.ItemTypeName, SubscribeDate, ContractDate, PaymentName,  "
                + " b.ItemID, b.Building, b.Unit, b.ItemNum, b.Area, b.Price, b.Amount, "
                + " DownPayRate, DownPayAmount, Loan, TotalAmount, a.SalesName, "
                ...
                + " where b.IsBind = 0 and a.ContractID in (" + condition + ")"
            ...
            //最后收款日期
            sql = string.Format("select ContractID, MAX(RecDate) as ReceiptDate from Receipt  where  ContractID in ({0}) group by ContractID", condition);
            ...
            sql = "select ContractID, "
                ...
                + " (SUM(case TypeCode when '1' then amount else 0 end) + SUM(case TypeCode when '2' then amount else 0 end)) RecDownPay,"
        ...
        private bool IsSetted(string contractId)
        {
            ...
            if (result > 0)  //是否存分期
            {
                ...
                if ((int.Parse(SqlHelper.ExecuteScalar(sql).ToString()) == 0))  //是否全部结算
                {
                    return true;
                }
            }
        ...
        private bool SaveSettleData(string tabMaker, string tabDate, string tabName)
        {
            ...
            using (SqlConnection connection = SqlHelper.OpenConnection())  //创建连接对象 
            {
                ...
                try
                {
                    ...
                    string fields = "insert into SettleDetail (SettleID,ContractID, ContractNum, CustomerName,ItemTypeName, SubscribeDate, ContractDate,PaymentName,SalesName,"
